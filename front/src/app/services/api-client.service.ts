...
export class ApiClientService {
  ...
  private baseBackendUrl = 'http://localhost:8080/api/fake';
  ...
  getFakeApi(request: AttackInput) : Observable<boolean> {
    return this.client.get<AttackOutput>(this.baseBackendUrl).pipe(
  ...
  postFakeApi(request: AttackInput) : Observable<boolean> {
    return this.client.post<AttackOutput>(this.baseBackendUrl, request).pipe(
      map((response: AttackOutput) => {
        setTimeout(() => {
          this.apiSubject$.next(response);
        }, 2000); // Simulate a delay of 1 second
